...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeAmsterdamCoinAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if(!sessionFoundMasternode){
        ...
        double fAmsterdamCoinSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f AMS CENT - sessionTotalValue %d\n", fAmsterdamCoinSubmitted, sessionTotalValue);
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    nAnonymizeAmsterdamCoinAmount = (rand() % nLeftToAnon)+3;
                } else {
